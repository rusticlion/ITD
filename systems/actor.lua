...
local function object_tile_position(data, tile_size)
...
function Actor.new(data, room)
    data = data or {}
    local properties = copy_table(data.properties or {})
    local actor_type = properties.actor_type or data.actor_type or data.type or "message"
    local x, y = object_tile_position(data, room and room.tile_size or DEFAULT_TILE_SIZE)
    local id = properties.id or data.name or data.id or (actor_type .. "_" .. tostring(x) .. "_" .. tostring(y))
